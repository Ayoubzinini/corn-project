...
while True:
    ...
    model.fit(x_train, y_train)
    RMSECV=abs(mean(cross_val_score(PLSRegression(n_components=5), x_train, y_train, scoring='neg_root_mean_squared_error', cv=LeaveOneOut())))
    R2CV=100*mean(cross_val_score(PLSRegression(n_components=5), x_train, y_train, scoring='r2'))
